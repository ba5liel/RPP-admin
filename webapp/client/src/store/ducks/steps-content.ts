...
export const ADD_JT = 'steps-content/ADD_JT';
...
export const ADD_CHANNEL = 'steps-content/ADD_CHANNEL';
...
const reducer = (state = DEFAULT_STATE, action: IAction): IStepsContent => {
  switch (action.type) {
    case ADD_JT:
      return { ...state, jt: action.payload };
    case ADD_JD:
...
// Action Creators
export const addJt = (jt: string): IAction => {
  return { type: ADD_JT, payload: jt };
};
...
export const addChannel = (channels: string[]): IAction => {
  return { type: ADD_RESUME, payload: channels };
};
